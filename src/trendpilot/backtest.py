...
def run_event_backtest(
    ...
    bench = normalize_ohlcv(benchmark)
    ...
    while cursor < len(data) - 1:
        ...
        trades.append(
            BacktestTrade(
                symbol=symbol,
                signal_date=str(index[cursor]),
                entry_date=str(index[entry_idx]),
                exit_date=str(index[exit_idx]),
...
def summarize(trades: list[BacktestTrade]) -> dict:
    ...
    return {
        ...
        "total_compounded_return": round(compounded - 1, 6),
        "details": [asdict(trade) for trade in trades],
    }


def run_vectorbt(close: pd.Series, entries: pd.Series, exits: pd.Series):
    """Optional VectorBT integration for large parameter studies."""
    try:
